...
[What is a Privilege? →](/privileges)

## Run an agent with its own WaaP wallet

For developers building autonomous agents, WaaP ships a headless CLI that gives the agent its own WaaP account and address. Same signing model as a personal WaaP wallet, with the same Privileges and policy controls.
...
- **The agent holds no raw private keys.** On its machine sit the CLI and a session token, nothing that can sign. Hijack it completely and every request still has to clear your policy.
